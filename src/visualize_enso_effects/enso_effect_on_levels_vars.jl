...
"""
This script will:
visualize the effect of ENSO (ONI) on various atmospheric variables
...
It will do so by calculating correlations between ONI at lags -6, -3, 0, 3, and 6 and the variable of interest; it will also do 1-component and 3-component PLS regressions
It will save plots to vis/enso_effects/gridded_vars/
"""

visdir = "../../vis/enso_radiation_effects/gridded_vars/"
if !isdir(visdir)
    mkpath(visdir)
end
...
for (stacked_var_name, stacked_level_var) in zip(stacked_var_names, stacked_level_vars)
    ...
    for lag_name in oni_lag_names
        ...
        oni_data = enso_data[lag_name]
        simple_corrs = cor.(eachslice(vertical_concat_data, dims = (1,2,3)), Ref(oni_data))
    ...
    # Create text file with X weights and corresponding ONI lag information
    ...
    open(x_weights_file, "w") do io
        ...
        println(io, "=" ^ 50)
        println(io, "Component 1 weights showing contribution of each ONI lag:")
        println(io, "")
        for (i, lag) in enumerate(oni_lags)
            println(io, "ONI lag $lag months: $(X_weights[i])")
        end
        println(io, "")
        println(io, "Note: Larger absolute values indicate stronger contribution")
        println(io, "Positive values: Variable increases with positive ONI (El Niño)")
        println(io, "Negative values: Variable decreases with positive ONI (La Niña effect)")
    end
    
    println("Saved X weights to: $x_weights_file")

    # Plot correlations for all ONI lags and levels in one large plot
    ...
    for (i, lag_name) in enumerate(oni_lag_names)
        for (j, level) in enumerate(level_names)
            ...
            push!(all_corr_subtitles, "$level - ONI lag $(oni_lags[i])")
        end
    end
    
    # Create one large plot with all correlations (5 lags × 5 levels = 25 subplots)
    layout = (length(oni_lags), length(level_names))  # 5 rows (lags) × 5 columns (levels)
    ...
    corr_fig = plot_multiple_levels(lat, lon, all_corr_slices, layout;
                                   ...
                                   colorbar_label="Correlation with ONI")
    
    # Save the comprehensive correlation plot
    corr_fig.suptitle("$(stacked_var_name) - ENSO Correlations (Rows: ONI Lags, Columns: Levels)", fontsize=16)
...
for var_name in single_vars_era
    ...
    for lag_name in oni_lag_names
        oni_data = enso_data[lag_name]
        simple_corrs = cor.(eachslice(var_data, dims = (1,2)), Ref(oni_data))
    ...
    corr_subtitles = ["ONI lag $(oni_lags[i])" for i in 1:length(oni_lags)]
    
    # Plot correlations (1 row × 5 lags)
    layout = (1, length(oni_lags))
    corr_fig = plot_multiple_levels(lat, lon, corr_slices_float, layout;
                                   subtitles=corr_subtitles,
                                   colorbar_label="Correlation with ONI")
    
    corr_fig.suptitle("$(var_name) - ENSO Correlations", fontsize=16)
...
for var_name in single_vars_ceres
    ...
    if ndims(var_data) == 1
        ...
        for lag_name in oni_lag_names
            oni_data = enso_data[lag_name]
            corr_val = cor(var_data, oni_data)
        ...
        open(results_file, "w") do io
            ...
            println(io, "=" ^ 60)
            println(io, "")
            println(io, "CORRELATIONS WITH ONI:")
            for (i, lag) in enumerate(oni_lags)
                println(io, "ONI lag $lag months: $(lag_correlations[i])")
            end
            println(io, "")
