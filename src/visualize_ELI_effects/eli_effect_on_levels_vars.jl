...
"""
This script will:
visualize the effect of ELI (ENSO Longitude Index) on various atmospheric variables
...
It will do so by calculating correlations between ELI at lags -6, -3, 0, 3, and 6 and the variable of interest; it will also do 1-component and 3-component PLS regressions
It will save plots to vis/eli_effects/gridded_vars/
"""

visdir = "../../vis/eli_radiation_effects/gridded_vars/"
if !isdir(visdir)
    mkpath(visdir)
end
...
eli_lags = [-6, -3, 0, 3, 6]
eli_lag_names = ["ELI_Lag$lag" for lag in eli_lags]
...
#Prep it
times = eli_coords["time"]
float_times = @. year(times) + (month(times) - 1) / 12
month_groups = groupfind(month.(times))
for eli_col in eli_lag_names
    detrend_and_deseasonalize_precalculated_groups!(eli_data[eli_col], float_times, month_groups)
end
eli_data_block = hcat([eli_data[col] for col in eli_lag_names]...) #Block data for PLS
...
for (stacked_var_name, stacked_level_var) in zip(stacked_var_names, stacked_level_vars)
    ...
    #Now for each ELI lag calculate the correlations for each slice of this array
    names_arr = String[]
    slices_arr = []
    for lag_name in eli_lag_names
        #Calculate the correlations on each level
        eli_data_var = eli_data[lag_name]
        simple_corrs = cor.(eachslice(vertical_concat_data, dims = (1,2,3)), Ref(eli_data_var))
    ...
    # Create text file with X weights and corresponding ELI lag information
    ...
    open(x_weights_file, "w") do io
        ...
        println(io, "=" ^ 50)
        println(io, "Component 1 weights showing contribution of each ELI lag:")
        println(io, "")
        for (i, lag) in enumerate(eli_lags)
            println(io, "ELI lag $lag months: $(X_weights[i])")
        end
        println(io, "")
        println(io, "Note: Larger absolute values indicate stronger contribution")
        println(io, "Positive values: Variable increases with positive ELI")
        println(io, "Negative values: Variable decreases with positive ELI")
    end
    
    println("Saved X weights to: $x_weights_file")

    # Plot correlations for all ELI lags and levels in one large plot
    ...
    for (i, lag_name) in enumerate(eli_lag_names)
        for (j, level) in enumerate(level_names)
            ...
            push!(all_corr_subtitles, "$level - ELI lag $(eli_lags[i])")
        end
    end
    
    # Create one large plot with all correlations (5 lags × 5 levels = 25 subplots)
    layout = (length(eli_lags), length(level_names))  # 5 rows (lags) × 5 columns (levels)
    ...
    corr_fig = plot_multiple_levels(lat, lon, all_corr_slices, layout;
                                   ...
                                   colorbar_label="Correlation with ELI")
    
    # Save the comprehensive correlation plot
    corr_fig.suptitle("$(stacked_var_name) - ELI Correlations (Rows: ELI Lags, Columns: Levels)", fontsize=16)
...
for var_name in single_vars_era
    ...
    # Calculate correlations for each ELI lag
    names_arr = String[]
    slices_arr = []
    for lag_name in eli_lag_names
        eli_data_var = eli_data[lag_name]
        simple_corrs = cor.(eachslice(var_data, dims = (1,2)), Ref(eli_data_var))
    ...
    corr_subtitles = ["ELI lag $(eli_lags[i])" for i in 1:length(eli_lags)]
    
    # Plot correlations (1 row × 5 lags)
    layout = (1, length(eli_lags))
    corr_fig = plot_multiple_levels(lat, lon, corr_slices_float, layout;
                                   subtitles=corr_subtitles,
                                   colorbar_label="Correlation with ELI")
    
    corr_fig.suptitle("$(var_name) - ELI Correlations", fontsize=16)
...
for var_name in single_vars_ceres
    ...
    if ndims(var_data) == 1
        ...
        for lag_name in eli_lag_names
            eli_data_var = eli_data[lag_name]
            corr_val = cor(var_data, eli_data_var)
        ...
        open(results_file, "w") do io
            ...
            println(io, "=" ^ 60)
            println(io, "")
            println(io, "CORRELATIONS WITH ELI:")
            for (i, lag) in enumerate(eli_lags)
                println(io, "ELI lag $lag months: $(lag_correlations[i])")
            end
            println(io, "")
