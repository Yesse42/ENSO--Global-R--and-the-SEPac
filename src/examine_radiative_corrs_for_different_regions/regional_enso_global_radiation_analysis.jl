...
println("Starting regional ENSO - global radiation analysis...")
...
# Load global CERES radiation data
println("Loading global CERES radiation data...")
ceres_variables = ["gtoa_lw_all_mon", "gtoa_net_all_mon", "gtoa_sw_all_mon"]
ceres_data, ceres_coords = load_ceres_data(ceres_variables, time_period)
ceres_time = ceres_coords["time"]

# Convert CERES data to DataFrame and apply detrending/deseasonalizing
ceres_df = DataFrame()
ceres_df.Date = [Date(year(dt), month(dt), 1) for dt in ceres_time]

# Process each radiation variable
for (i, var) in enumerate(ceres_variables)
    if haskey(ceres_data, var)
        # Get the raw data
        raw_data = copy(ceres_data[var])
        
        # Convert dates to float times for detrending
        float_times = [calc_float_time(DateTime(dt)) for dt in ceres_df.Date]
        months = [month(dt) for dt in ceres_df.Date]
        
        # Apply detrending and deseasonalizing
        processed_data, fit = detrend_and_deseasonalize!(raw_data, float_times, months)
        
        # Create column names based on variable names
        if var == "gtoa_lw_all_mon"
            col_name = "Global_TOA_LW_Detrend_Deseas"
        elseif var == "gtoa_net_all_mon"
            col_name = "Global_TOA_Net_Detrend_Deseas"
        elseif var == "gtoa_sw_all_mon"
            col_name = "Global_TOA_SW_Detrend_Deseas"
        else
            col_name = "$(var)_Detrend_Deseas"
        end
        
        ceres_df[!, Symbol(col_name)] = processed_data
        
        println("  Processed $var -> $col_name (slope: $(round(fit.slope, digits=6)))")
    end
end

println("Global CERES data loaded and processed. $(nrow(ceres_df)) time points available.")
...
vis_dir = "../../vis/radiative_corrs_different_regions/global_rad"
if !isdir(vis_dir)
    mkpath(vis_dir)
end
...
for region_name in region_names
    ...
    if !isfile(t2m_file)
        @warn "T2M data file not found for region $region_name, skipping..."
        continue
    end
    ...
    println("  Global radiation date range: $(minimum(ceres_df.Date)) to $(maximum(ceres_df.Date))")
    ...
    # First merge T2M and global radiation data
    merged_df = DataFrames.innerjoin(t2m_df[:, [:Date, :T2M_Detrend_Deseas]], ceres_df, on=:Date)
    ...
    # Correlations with global radiation variables
    global_sw_corr_t2m, _ = calculate_lagged_correlations(merged_df.Global_TOA_SW_Detrend_Deseas, merged_df, "T2M")
    global_lw_corr_t2m, _ = calculate_lagged_correlations(merged_df.Global_TOA_LW_Detrend_Deseas, merged_df, "T2M")
    global_net_corr_t2m, _ = calculate_lagged_correlations(merged_df.Global_TOA_Net_Detrend_Deseas, merged_df, "T2M")
    
    global_sw_corr_oni, _ = calculate_lagged_correlations(merged_df.Global_TOA_SW_Detrend_Deseas, merged_df, "oni")
    global_lw_corr_oni, _ = calculate_lagged_correlations(merged_df.Global_TOA_LW_Detrend_Deseas, merged_df, "oni")
    global_net_corr_oni, _ = calculate_lagged_correlations(merged_df.Global_TOA_Net_Detrend_Deseas, merged_df, "oni")
    ...
    global_sw_corr_resid, _ = calculate_lagged_correlations(merged_df.Global_TOA_SW_Detrend_Deseas, residual_lagged_df, "T2M_Residual")
    global_lw_corr_resid, _ = calculate_lagged_correlations(merged_df.Global_TOA_LW_Detrend_Deseas, residual_lagged_df, "T2M_Residual")
    global_net_corr_resid, _ = calculate_lagged_correlations(merged_df.Global_TOA_Net_Detrend_Deseas, residual_lagged_df, "T2M_Residual")
    ...
    all_correlations = vcat(
        global_sw_corr_t2m, global_lw_corr_t2m, global_net_corr_t2m,
        global_sw_corr_oni, global_lw_corr_oni, global_net_corr_oni,
        global_sw_corr_resid, global_lw_corr_resid, global_net_corr_resid
    )
    ...
    p1 = plot(title="$region_name: Global Radiation vs T2M Correlations", 
              xlabel="Lag (months)", ylabel="Correlation", 
              ylims=(y_min, y_max), legend=:topright)
    plot!(p1, collect(lags), global_sw_corr_t2m, label="Global TOA SW", color=:orange, linewidth=2)
    plot!(p1, collect(lags), global_lw_corr_t2m, label="Global TOA LW", color=:red, linewidth=2)
    plot!(p1, collect(lags), global_net_corr_t2m, label="Global TOA Net", color=:blue, linewidth=2)
    hline!(p1, [0], color=:black, linestyle=:dash, alpha=0.5, label="")
    
    # Plot 2: Correlations with ONI
    p2 = plot(title="$region_name: Global Radiation vs ONI Correlations", 
              xlabel="Lag (months)", ylabel="Correlation", 
              ylims=(y_min, y_max), legend=:topright)
    plot!(p2, collect(lags), global_sw_corr_oni, label="Global TOA SW", color=:orange, linewidth=2)
    plot!(p2, collect(lags), global_lw_corr_oni, label="Global TOA LW", color=:red, linewidth=2)
    plot!(p2, collect(lags), global_net_corr_oni, label="Global TOA Net", color=:blue, linewidth=2)
    hline!(p2, [0], color=:black, linestyle=:dash, alpha=0.5, label="")
    
    # Plot 3: Correlations with Residual T2M
    p3 = plot(title="$region_name: Global Radiation vs Residual T2M Correlations", 
              xlabel="Lag (months)", ylabel="Correlation", 
              ylims=(y_min, y_max), legend=:topright)
    plot!(p3, collect(lags), global_sw_corr_resid, label="Global TOA SW", color=:orange, linewidth=2)
    plot!(p3, collect(lags), global_lw_corr_resid, label="Global TOA LW", color=:red, linewidth=2)
    plot!(p3, collect(lags), global_net_corr_resid, label="Global TOA Net", color=:blue, linewidth=2)
    ...
    # Save plot
    plot_filename = joinpath(vis_dir, "$(region_name)_global_radiation_correlations.png")
    ...
    corr_data = DataFrame(
        Lag = collect(lags),
        Global_TOA_SW_vs_T2M = global_sw_corr_t2m,
        Global_TOA_LW_vs_T2M = global_lw_corr_t2m,
        Global_TOA_Net_vs_T2M = global_net_corr_t2m,
        Global_TOA_SW_vs_ONI = global_sw_corr_oni,
        Global_TOA_LW_vs_ONI = global_lw_corr_oni,
        Global_TOA_Net_vs_ONI = global_net_corr_oni,
        Global_TOA_SW_vs_Residual_T2M = global_sw_corr_resid,
        Global_TOA_LW_vs_Residual_T2M = global_lw_corr_resid,
        Global_TOA_Net_vs_Residual_T2M = global_net_corr_resid
    )
    
    corr_filename = joinpath(vis_dir, "$(region_name)_global_radiation_correlation_data.csv")
    ...
    summary_filename = joinpath(vis_dir, "$(region_name)_global_radiation_analysis_summary.csv")
    CSV.write(summary_filename, summary_data)
    
    println("  Analysis complete for $region_name")
end

println("\nRegional ENSO - global radiation analysis completed!")
