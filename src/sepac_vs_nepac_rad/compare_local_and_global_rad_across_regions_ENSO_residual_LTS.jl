"""
This script aims to calculate the correlation between local radiation and ENSO residual LTS (Lower Tropospheric Stability), 
and global radiation and ENSO residual LTS, across the three stratocumulus regions (SEPac, NEPac, SEAtl).
"""
...
for (j, region) in enumerate(regions)
    ...
    for (i, (rad_data, rad_names)) in enumerate(local_global_rad)
        p = plot(title = "$(region) - $(i == 1 ? "Global" : "Local") Radiation vs. ENSO Residual LTS", xlabel = "Lag (months), radiation lags to right", ylabel = "(Weighted) Correlation", legend = :topright, size = (600, 400), titlefontsize = 8, yticks = -1:0.05:1)
        ...
        for (rad, rad_name) in zip(rad_data, rad_names)
            corrs = Float64[]
            for lag in lags
                lagged_lts_enso_removed = lts_enso_removed_df[!, "LTS_detrend_deseason_enso_removed_lag_$(lag)"]
                valid_indices = .!(ismissing.(rad) .| ismissing.(lagged_lts_enso_removed))
                if sum(valid_indices) > 0
                    push!(corrs, cor(rad[valid_indices], lagged_lts_enso_removed[valid_indices]))
                else
                    push!(corrs, NaN)
                end
            end
...
ylims!.(plot_arr, Ref((-0.75, 0.75)))

final_plot = plot(permutedims(plot_arr)..., layout = (2, length(region_names)), size = (1800, 800); plot_title = "Correlation between Local and Global Radiation vs. ENSO Residual LTS across Stratocumulus Regions", titlefontsize = 8)

savefig(joinpath(visdir, "local_vs_global_rad_correlation_across_regions_ENSO_residual_LTS.png"))
