...
This version uses ENSO residual data for temperature instead of lagged data.
"""
...
for (j, region) in enumerate(regions)
    ...
    # Use ENSO residual data instead of lagged data
    t2m_enso_residual_df = CSV.read(joinpath(datadir, "era5_region_avg_enso_removed_$(region).csv"), DataFrame)
    filter!(row -> is_analysis_time(row.date), t2m_enso_residual_df)
    
    for (i, (rad_data, rad_names)) in enumerate(local_global_rad)
        p = plot(title = "$(region) - $(i == 1 ? "Global" : "Local") Radiation vs. T2M (ENSO Residual)", xlabel = "Lag (months), radiation lags to right", ylabel = "(Weighted) Correlation", legend = :topright, size = (600, 400), titlefontsize = 8, yticks = -1:0.05:1)
        ...
        for (rad, rad_name) in zip(rad_data, rad_names)
            corrs = Float64[]
            for lag in lags
                # Use ENSO residual column names
                lagged_t2m = t2m_enso_residual_df[!, "t2m_detrend_deseason_enso_removed_lag_$(lag)"]
...
final_plot = plot(permutedims(plot_arr)..., layout = (2, length(region_names)), size = (1800, 800); plot_title = "Correlation between Local and Global Radiation vs. T2M (ENSO Residual) across Stratocumulus Regions", titlefontsize = 8)

savefig(joinpath(visdir, "local_vs_global_rad_correlation_across_regions_enso_residual.png"))
